...
3. Honor the blog-specific first step. Hot Mess: run the pre-generation check. Clean Enough: run the schedule check (it determines the post's format). Irene: run SEO discovery (unless told "skip seo"), then INTERVIEW the user before drafting. Do not draft Irene posts from assumptions.
